...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	...
	if (!(perm & PTE_COW)) {
		user_panic("cow_entry called without PTE_COW perm.\n");
	}
	...
	r = syscall_mem_alloc(0, (void *)UCOW, perm);
	if (r != 0) {
		user_panic("syscall_mem_alloc bad with return val %d.\n", r);
	}
	...
	r = syscall_mem_map(0, (void *)UCOW, 0, (void *)va, perm);
	if (r != 0) {
		user_panic("syscall_mem_map bad with return val %d.\n", r);
	}

	// Step 6: Unmap the page at 'UCOW'.
	/* Exercise 4.13: Your code here. (6/6) */

	r = syscall_mem_unmap(0, (void *)UCOW);
	if (r != 0) {
		user_panic("syscall_mem_unmap bad with return val %d.\n", r);
	}
...
static void duppage(u_int envid, u_int vpn) {
	...
	perm = vpt[vpn] & 0xfff;
	addr = vpn * BY2PG; // not understand why

	// user_panic("duppage envid %u vpn %u perm %u addr %u.\n", envid, vpn, perm,
	// 		addr);
	...
	void dup_pnc(char *msg) { 
		if (r != 0) user_panic("In fork.c/duppage, mem_map failed on %s with \
				error message r = %d.\n", msg, r);
	}
	...
	if (!(perm & PTE_D)) {
		...
		user_panic("I don\'t think here should run.\n");
		r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
		dup_pnc("with PTE_COW");
	}
	else if (perm & PTE_LIBRARY) {
		r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
		dup_pnc("with PTE_LIBRARY");
	}
	else {
		perm = (perm | PTE_COW) & (~PTE_D);
		r = syscall_mem_map(0, (void *)addr, envid, (void *)addr, perm);
		dup_pnc("son add PTE_COW");
		r = syscall_mem_map(0, (void *)addr, 0, (void *)addr, perm);
		dup_pnc("father add PTE_COW");
	}
}
...
int fork(void) {
	...
	for (i = 0; i < USTACKTOP; i += BY2PG) {
		if ((vpd[PDX(i)] & PTE_V) && (vpt[VPN(i)] & PTE_V)) {
			duppage(child, VPN(i));
			// user_panic("finished duppage i = %u, VPN(i) = %u.\n", i, VPN(i));
		}
	}
