#include <lib.h>

static char *msg = "This is the NEW message of the day!\n";
static char *diff_msg = "This is a different message of the day!\n";

int check5_4() {
	int r;
	int fdnum;
	char buf[512];
	int n;

	if ((r = fat_open("/newmotd_4", O_RDWR)) < 0) {
		user_panic("open /newmotd: %d", r);
	}
	fdnum = r;
	debugf("open is good\n");

	if ((n = read(fdnum, buf, 511)) < 0) {
		user_panic("read /newmotd: %d", n);
	}
	if (strcmp(buf, diff_msg) != 0) {
		user_panic("read returned wrong data");
	}
	debugf("read is good\n");

	if ((r = fat_truncate(fdnum, 1)) < 0) {
		user_panic("ftruncate: %d", r);
	}
	seek(fdnum, 0);

	int next_n = strlen(msg) + 1;
	if ((r = write(fdnum, msg, next_n) < 0)) {
		user_panic("write /newmotd: %d", r);
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("close /newmotd: %d", r);
	}

	// read again
	if ((r = fat_open("/newmotd_4", O_RDONLY)) < 0) {
		user_panic("open /newmotd: %d", r);
	}
	fdnum = r;
	debugf("open again: OK\n");

	if ((n = read(fdnum, buf, 511)) < 0) {
		user_panic("read /newmotd: %d", r);
	}
	if (strcmp(buf, msg) != 0) {
		user_panic("read returned wrong data");
	}
	debugf("read again: OK\n");

	if ((r = close(fdnum)) < 0) {
		user_panic("close /newmotd: %d", r);
	}

	debugf("file rewrite is good\n");

	if ((r = fat_open("/newmotd_4", O_RDONLY)) < 0) {
		user_panic("open /newmotd: %d", r);
	}
	fdnum = r;
	memset(buf, 0xfe, sizeof buf);
	if ((n = read(fdnum, buf, sizeof buf)) != next_n) {
		user_panic("read /newmotd: %d (%d expected)", n, next_n);
	}
	if (buf[next_n - 1] != 0) {
		user_panic("read /newmotd: zero byte not written");
	}
	int k = 0;
	for (k = next_n; k < sizeof buf; k++) {
		if ((u_char)buf[k] != 0xfe) {
			user_panic("read /newmotd: buffer overflow: %x", (u_char)buf[k]);
		}
	}
	if ((r = close(fdnum)) < 0) {
		user_panic("close /newmotd: %d", r);
	}

	debugf("1 OK\n");

	// test fd:
	for (k = 0; k < 31; k++) {
		if ((r = fat_open("/newmotd_4", O_RDWR)) < 0) {
			user_panic("not enough fd");
		}
		if (r != k) {
			user_panic("fdnum mismatch");
		}
	}

	for (k = 30; k >= 0; k--) {
		if ((r = close(k)) < 0) {
			user_panic("cannot close fd");
		}
	}

	if ((r = fat_open("/newmotd_4", O_RDWR)) < 0 || r != 0) {
		user_panic("close without free fd");
	}

	if ((n = read(fdnum, buf, 511)) < 0) {
		user_panic("read /newmotd: %d", r);
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("canot close fd");
	}

	if ((n = read(fdnum, buf, 511)) > 0) {
		user_panic("read on a closed file: %d", r);
	}

	if ((r = fat_open("/newmotd_4", O_WRONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("cannot close fd");
	}

	debugf("2 OK\n");

	// test limit of autority:
	fdnum = r;
	if ((n = read(fdnum, buf, 511)) >= 0) {
		user_panic("read on a file withou permission");
	}

	if ((r = fat_open("/newmotd_4", O_WRONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;

	if ((n = read(fdnum, buf, 511)) >= 0) {
		user_panic("read on a file withou permission: %d", r);
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("cannot close fd");
	}

	if ((r = fat_open("/newmotd_4", O_RDONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;

	if ((r = write(fdnum, msg, strlen(msg) + 1)) >= 0) {
		user_panic("write on a file without permission d: %d", r);
	}

	debugf("3 OK\n");

	// test remove
	if ((r = close(fdnum)) < 0) {
		user_panic("cannot close fd");
	}

	if ((r = fat_remove("/newmotd_4")) < 0) {
		user_panic("remove /newmotd: %d", r);
	}
	if ((r = fat_open("/newmotd_4", O_RDONLY)) >= 0) {
		user_panic("open after remove /newmotd: %d", r);
	}
	debugf("file remove: OK\n");

	debugf("4 OK\n");

	return 0;
}


static char *file1 = "/newmotd";
static char *file2 = "/motd";
static char *file3 = "/bin/test";
static char *file4 = "/etc/profile";

void test_read(char *pth, char *str) {
	char buf[1024];
	int r = fat_open(pth, O_RDWR);
	if (r < 0) {
		user_panic("failed to open %s, return value: %d", pth, r);
	}
	int len = strlen(str);
	fprintf(r, str, len);
	close(r);
	r = fat_open(pth, O_RDWR);
	if (r < 0) {
		user_panic("failed to open %s, return value: %d", pth, r);
	}
	int l = read(r, buf, len);
	if (l != len) {
		user_panic("wrong length, expect: %d, we got: %d", len, l);
	}
	buf[len] = '\0';
	if (strcmp(str, buf) != 0) {
		debugf("test read failed, expect: %s, we got: %s\n", str, buf);
		return;
	}
	close(r);
	debugf("passed test_read with strlen=%d, file=%s\n", strlen(str), pth);
}

void test_fd_alloc(char *pth) {
	int k = 0, r = 0;
	for (k = 0; k < 31; k++) {
		if ((r = fat_open(pth, O_RDWR)) < 0) {
			user_panic("not enough fd");
		}
		if (r != k) {
			user_panic("fdnum mismatch, expect: %d, we got: %d", k, r);
		}
	}

	for (k = 30; k >= 0; k--) {
		if ((r = close(k)) < 0) {
			user_panic("cannot close fd");
		}
	}

	debugf("pass fd alloc test with file=%s\n", pth);
}

void test_close(char *pth) {
	int r, fdnum, n;
	char buf[512];

	if ((r = fat_open(pth, O_RDWR)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;
	if ((n = read(fdnum, buf, 511)) < 0) {
		user_panic("read /newmotd: %d", r);
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("canot close fd");
	}

	if ((n = read(fdnum, buf, 511)) > 0) {
		user_panic("read on a closed file: %d", r);
	}

	if ((r = fat_open(pth, O_WRONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("canot close fd");
	}
	debugf("pass close test with file=%s\n", pth);
}

void test_mode(char *pth) {
	int r, fdnum, n;
	char buf[512];
	buf[0] = 'a';

	if ((r = fat_open(pth, O_WRONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;

	if ((n = read(fdnum, buf, 511)) >= 0) {
		user_panic("read on a file withou permission: %d", r);
	}

	if ((r = close(fdnum)) < 0) {
		user_panic("canot close fd");
	}

	if ((r = fat_open(pth, O_RDONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;

	if ((r = write(fdnum, buf, strlen(buf) + 1)) >= 0) {
		user_panic("write on a file without permission d: %d", r);
	}

	close(fdnum);

	debugf("pass permission test with file=%s\n", pth);
}

void test_fat_remove(char *pth) {
	int r, fdnum;

	if ((r = fat_open(pth, O_WRONLY)) < 0 || r != 0) {
		user_panic("close without free fd");
	}
	fdnum = r;

	if ((r = close(fdnum)) < 0) {
		user_panic("canot close fd");
	}

	if ((r = fat_remove(pth)) < 0) {
		user_panic("remove %d", r);
	}
	if ((r = fat_open(pth, O_RDONLY)) >= 0) {
		user_panic("open after remove: %d", r);
	}
	debugf("pass remove test with file=%s\n", pth);
}

int check5_5() {
	char test_str1[1 + 1];
	char test_str2[128 + 1];
	char test_str3[512 + 1];
	int i = 0;
	for (i = 0; i < 1; i++) {
		test_str1[i] = 'a';
	}
	test_str1[1] = 0;

	for (i = 0; i < 128; i++) {
		test_str2[i] = 'b';
	}

	test_str2[128] = 0;

	for (i = 0; i < 512; i++) {
		test_str3[i] = 'c';
	}
	test_str3[512] = 0;

	char *files[4] = {file1, file2, file3, file4};
	for (i = 0; i < 4; i++) {
		// debugf("file:%s\n", files[i]);
		test_read(files[i], test_str1);
		test_read(files[i], test_str2);
		test_read(files[i], test_str3);
	}

	for (i = 0; i < 4; i++) {
		test_fd_alloc(files[i]);
	}

	for (i = 0; i < 4; i++) {
		test_close(files[i]);
	}

	for (i = 0; i < 4; i++) {
		test_mode(files[i]);
	}

	for (i = 0; i < 4; i++) {
		test_fat_remove(files[i]);
	}
	int r;
	return 0;
}

int check5_chlg() {
	char buffs[1000];
	u_int fsfd = open("/bin/test", O_RDWR);
	user_assert(readn(fsfd, buffs, 1000) == 0);
	write(fsfd, msg, strlen(msg));
	close(fsfd);

	fat_create("/abc.c", 0, 0);
	fat_create("/bbb", FAT_ATTR_DIRECTORY, 128);
	fat_create("/bbb/ccc.c", 0, 8192);
	u_int fdccc = fat_open("/bbb/ccc.c", O_RDWR);
	u_int file_size = 8192;
	char buf[16384];
	for (int i = 0; i < 128; i++) {
		for (int j = 0; j < 64; j++) {
			if (j < 10) buf[i * 64 + j] = '0' + j;
			else if (j < 36) buf[i * 64 + j] = 'a' - 10 + j;
			else if (j < 62) buf[i * 64 + j] = 'A' - 36 + j;
			else buf[i * 64 + j] = '\n';
		}
	}
	write(fdccc, buf, file_size);
	close(fdccc);
	memset(buf, 0, file_size);
	fdccc = fat_open("/bbb/ccc.c", O_RDWR);
	user_assert(readn(fdccc, buf, file_size) == file_size);
	for (int i = 0; i < 128; i++) {
		for (int j = 0; j < 64; j++) {
			if (j < 10) user_assert(buf[i * 64 + j] == '0' + j);
			else if (j < 36) user_assert(buf[i * 64 + j] == 'a' - 10 + j);
			else if (j < 62) user_assert(buf[i * 64 + j] == 'A' - 36 + j);
			else user_assert(buf[i * 64 + j] == '\n');
		}
	}
	close(fdccc);
	debugf("\n\nMultiple cluster read & write, create passed!\n\n");
	fdccc = fat_open("/my_lyrics.txt_test", O_RDONLY);
	memset(buf, 0, file_size);
	readn(fdccc, buf, 1683);
	debugf("%s", buf);

	fsfd = open("/bin/test", O_RDWR);
	user_assert(readn(fsfd, buffs, 1000) == strlen(msg));
	close(fsfd);
	return 0;
}

int main() {
	check5_4();
	check5_5();
	check5_chlg();
	return 0;
}
