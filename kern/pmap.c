...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	int error_code = pgdir_walk(pgdir, va, 1, &pte);
...
void swap_init() {
		LIST_INIT(&page_free_swapable_list);
			for (int i = SWAP_PAGE_BASE; i < SWAP_PAGE_END; i +=
					BY2PG) {
						struct Page *pp = pa2page(i);
								LIST_REMOVE(pp, pp_link);
										LIST_INSERT_HEAD(&page_free_swapable_list,
												pp, pp_link);
											}
}
...
u_char swap_disk[SWAP_DISK_NPAGE * BY2PG]
__attribute__((aligned(BY2PG)));
u_char swap_disk_used[SWAP_DISK_NPAGE];

static u_char *disk_alloc() {
		int alloc = 0;
			for (;alloc < SWAP_DISK_NPAGE && swap_disk_used[alloc];
					alloc++) {
						;
							}
